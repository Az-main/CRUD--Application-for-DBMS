...
from user_management import create_user, get_all_users, update_user, delete_user
from product_management import create_product, get_all_products, update_product, delete_product

# Page Configuration
st.set_page_config(
    page_title="CRUD Application",
    page_icon="📦",
    layout="wide"
)
...
page = st.sidebar.radio("Select Module", ["🏠 Home", "👤 User Management", "📦 Product Inventory"])

# ==================== HOME PAGE ====================
if page == "🏠 Home":
    st.markdown("<h1 class='main-header'>🎯 CRUD Application</h1>", unsafe_allow_html=True)
    st.markdown("---")
    
    st.write("### Welcome to the CRUD Application!")
    st.write("This application allows you to manage **Users** and **Products** using a MySQL database.")
...
# ==================== USER MANAGEMENT ====================
    ...
    tab1, tab2, tab3 = st.tabs(["➕ Add User", "📋 View Users", "✏️ Update/Delete"])
    ...
    with tab3:
        ...
        try:
            ...
            if users:
                ...
                current_user = [u for u in users if u[0] == user_id][0]
                
                col1, col2 = st.columns(2)
                
                with col1:
                    st.write("#### ✏️ Update User")
                    with st.form("update_user_form"):
                        new_username = st.text_input("Username", value=current_user[1])
                        new_email = st.text_input("Email", value=current_user[2])
                        new_password = st.text_input("New Password", type="password", placeholder="Leave blank to keep same")
                        update_btn = st.form_submit_button("Update User", type="primary")
                        
                        if update_btn:
                            pwd = new_password if new_password else current_user[3]
                            try:
                                update_user(user_id, new_username, new_email, pwd)
                ...
                st.info("No users found to update or delete.")
        except Exception as e:
            st.error(f"❌ Database Error: {e}")
...
# Footer
st.sidebar.markdown("---")
st.sidebar.info("📌 **CRUD Application**\nBuilt with Streamlit & MySQL")
